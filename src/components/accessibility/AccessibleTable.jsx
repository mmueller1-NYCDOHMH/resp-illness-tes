...
export default function AccessibleTable({
  ...
  return (
    <div className={srOnly && !open ? "sr-only" : "mt-3"}>
      {allowToggleForSighted && srOnly && (
        <button
          type="button"
          className="bg-transparent border border-[var(--gray-300)] rounded-md px-2 py-1 mb-2 font-[inherit] cursor-pointer transition-[background] duration-200 hover:bg-[var(--gray-100)] focus:bg-[var(--gray-100)]"
          aria-expanded={open}
      ...
      <table id={tableId} className="w-full border-collapse font-[inherit]" aria-labelledby={labelledBy}>
        {caption && (
          <caption className="text-left mb-1.5 font-semibold">
            {interpolate(caption, variables)}
          </caption>
        )}
        <thead>
          <tr>
            {safeCols.map((c) => (
              <th
                key={c.key}
                scope="col"
                className="border border-[var(--gray-300)] px-2 py-1.5 text-left align-top bg-[var(--gray-100)] font-semibold"
              >
            ...
            <tr key={i}>
              {safeCols.map((c) => {
                ...
                return (
                  <td
                    key={c.key}
                    className="border border-[var(--gray-300)] px-2 py-1.5 text-left align-top"
                  >
                    {f(row?.[c.key], row, c)}
                  </td>
                );
