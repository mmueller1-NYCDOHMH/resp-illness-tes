...
const toNum = (v) => {
  ...
  if (typeof v === "string") {
    ...
    if (cleaned.startsWith("<")) return null; // 👈 suppressed ≠ numeric
...
export function buildStyledTrendSentence({
  metricLabel = "Visits",
  latestWeek,
  trend,
  dateHtml,
  ...
  const formattedDate =
    dateHtml ||
    `<span class="bg-highlight">${latestWeek.toLocaleDateString("en-US", {
      month: "long",
      day: "numeric",
      year: "numeric",
    })}</span>`;
  ...
  if (currVal === "N/A" && prevVal === "N/A") {
    return `${metricLabel} data is not available for this week.`;
  }

  if (currVal === "N/A" && prevVal !== "N/A") {
    return `${metricLabel} have ${styledDir} — current value unavailable (previous week was ${prevChip}).`;
  }

  if (prevVal === "N/A" && currVal !== "N/A") {
    return `${metricLabel} are at ${currChip} this week — no prior week available for comparison.`;
  }
   ...
   * --------------------------------------------------------- */
  if (direction === "same") {
    return `${metricLabel} have ${styledDir} from ${prevChip} the previous week to ${currChip} this week.`;
  }
